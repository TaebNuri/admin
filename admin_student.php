...
if(isset($_GET['email'])){
    ...
    $sql = "UPDATE `candidates` SET `ok` = '$ok',`date` = current_timestamp() WHERE `candidates`.`email` = '$email' ";
...
if(isset($_GET['email1'])){
    ...
    $sql = "UPDATE `candidates` SET `ok` = '$value',`date` = '$value' WHERE `candidates`.`email` = '$email' ";
    $result = mysqli_query($conn, $sql);
            ...
            <table width="100%">
                ...
                <?php
                    ...
                    <tr>
                        ...
                        <td class="active">
                            <p><?php 
                                echo "<a href = 'admin_student_more.php?email=".$row['email']."' class='btn''><b>Details</b></a>"; 
                                ?></p>
                        </td>
                        ...
                        <?php 
                            if($row['ok']=='ok')
                            {
                                echo "<a href = 'admin_student.php?email1=".$row['email']."' class='btn btn-outline-success'>Activate</a>"; 
                            }
                            else
                            {
                                echo "<a href = 'admin_student.php?email=".$row['email']."' class='btn btn-outline-danger'>Deactivate</a>";
                            }
                            ?>
                        </td>
                    </tr>
